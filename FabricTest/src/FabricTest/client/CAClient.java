...
public class CAClient {
	...
	public UserContext reenrollUser(UserContext user,String secret)throws Exception{
		UserContext userContext = Util.readUserContext(adminContext.getAffiliation(), user.getName());
		if (userContext == null) {
			Logger.getLogger(CAClient.class.getName()).log(Level.WARNING, "CA -" + caUrl + " User " + user.getName()+" is not exited");
			return userContext;
		}
		Enrollment enrollment = instance.reenroll(userContext);
		user.setEnrollment(enrollment);
		Util.writeUserContext(user);
		Logger.getLogger(CAClient.class.getName()).log(Level.INFO, "CA -" + caUrl +" Reennrolled User - " + user.getName());
		return user;
	}
	
	/*public UserContext revokeUser(UserContext user)throws Exception{
		UserContext userContext = Util.readUserContext(adminContext.getAffiliation(), user.getName());
		if (userContext == null) {
			Logger.getLogger(CAClient.class.getName()).log(Level.WARNING, "CA -" + caUrl + " User " + user.getName()+" is not exited");
			return userContext;
		}
		Enrollment enrollment = instance.revoke(revoker, enrollment, reason);
		user.setEnrollment(enrollment);
		Util.writeUserContext(user);
		Logger.getLogger(CAClient.class.getName()).log(Level.INFO, "CA -" + caUrl +" Reennrolled User - " + user.getName());
		return user;
